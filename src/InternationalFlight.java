...
public class InternationalFlight extends JFrame
{
	...
	JLabel LFrom, LTo, LBookingDate, LClass, LAdult, LChildren, LInfant, LBookingDetails, LPassengerDetails, LDate, LImg1, LImg2;
	...
	public InternationalFlight(MainPage type1)
	{
		...
		String[] sItem1={"Nairobi"};
		String[] sItem2={"Bangkok","Cairo","CapeTown","Chicago","Dubai","Frankfurt","HongKong","Istanbul","London","LosAngeles","Melbourne","New York","Paris","Rome","SanFrancisco","shanghai","Singapore","Sydney","Toronto"};
		...
		LFrom.setBounds(236,56,100,20);
		CBFrom.setBounds(316,56,100,20);

		LTo.setBounds(236,104,100,20);
		CBTo.setBounds(316,104,100,20);

		LBookingDate.setBounds(128,177,100,20);
		TFBookingDate.setBounds(208,177,100,20);
		LDate.setBounds(316,177,100,20);

		LClass.setBounds(128,216,100,20);
		CBClass.setBounds(208,216,100,20);

		BFindFlight.setBounds(169,276,100,25);
		...
		lblFirstName.setBounds(20, 70, 83, 14);
		PPanel1.add(lblFirstName);
		
		lblLastName = new JLabel("Last Name    :");
		lblLastName.setBounds(20, 110, 83, 14);
		PPanel1.add(lblLastName);
		
		textField = new JTextField();
		textField.setBounds(63, 67, 18, -5);
		PPanel1.add(textField);
		textField.setColumns(10);
		
		txtfName = new JTextField();
		txtfName.setBounds(99, 67, 101, 20);
		PPanel1.add(txtfName);
		txtfName.setColumns(10);
		
		txtlName = new JTextField();
		txtlName.setColumns(10);
		txtlName.setBounds(99, 107, 101, 20);
		PPanel1.add(txtlName);
...
class button2 implements ActionListener
{
	...
	MainPage type1;
	button2(InternationalFlight type, MainPage type1)
	{
		this.type = type;
		this.type1 = type1;
	}
	...
	{
		String fName = type.txtfName.getText();
		String lName = type.txtlName.getText();
		...
		String[] sfName = new String[1250];
		String[] slName = new String[1250];
...
//read from data
			...
			do
			{
				...
				sfName[iCount] = save1.fName;
				slName[iCount] = save1.lName;
		...
		else
		{
			...
			{
				new PrintTicket1(fName, lName, sFrom, sTo, sClass, iAdult, iChildren, iInfant, sBookingDate, iPrice, sTime);
			try
			{
//write into data
				Save1 save2=new Save1(fName, lName, sFrom, sTo, sClass, iAdult, iChildren, iInfant, sBookingDate, iPrice, sTime);
				ObjectOutputStream OOS1 = new ObjectOutputStream(new FileOutputStream("save1"));
				for(i=0;i<iCount;i++)
				{
					Save1 temp1=new Save1(sfName[i],slName[i],  sTempFrom[i], sTempTo[i], sTempClass[i], iTempAdult[i], iTempChildren[i], iTempInfant[i], sTempBookingDate[i], iTempPrice[i], sTempTime[i]);
...
{
	String fName, lName, sFrom, sTo, sClass, sBookingDate, sTime;
	Integer iPrice, iAdult, iChildren, iInfant;
//	int iCount;
	public Save1( String fName, String lName, String sFrom, String sTo, String sClass, Integer iAdult, Integer iChildren, Integer iInfant, String sBookingDate, Integer iPrice, String sTime)
	{
		this.fName=fName;
		this.lName=lName;
		this.sFrom=sFrom;
